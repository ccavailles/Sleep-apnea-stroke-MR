...
stroke_path = "data/stroke_reformatted.tsv.gz"
stroke_out_dat <- read_tsv(stroke_path)
head(stroke_out_dat)

## Format data to TwoSampleMR format
outcome <- stroke_out_dat %>%
  format_data(.,
              type = "outcome",
              snps = NULL,
              header = TRUE,
...
exposure1_stroke_bmi_proxy_clumb <- exposure1_stroke_bmi_proxy %>% 
  clump_data(.,
             clump_kb = 10000,
             clump_r2 = 0.001,
             clump_p1 = 1,
...
exposure1_stroke_bmi_proxy_clumb_independent <- comb_exp_proxy %>% filter(SNP %in% 
                                                                            exposure1_stroke_bmi_proxy_clumb$SNP)
...
exposure1_stroke_bmi_proxy_clumb_independent  <- filter(comb_exp_proxy, SNP %in% exposure1_stroke_bmi_proxy_clumb$rsid) 

write_tsv(exposure1_stroke_bmi_proxy_clumb_independent, 
          "data/combined_exposures.tsv.gz")

## Can start here
exposure1_stroke_bmi_proxy_clumb_independent <- read_tsv("data/combined_exposures.tsv.gz")
...
outcome_clump <- semi_join(
  outcome, exposure1_stroke_bmi_proxy_clumb_independent, by = "SNP"
)

# Exposure SNPs not present in outcome
outcome_wo_snps <- anti_join(
  exposure1_stroke_bmi_proxy_clumb_independent, outcome, by = "SNP"
) %>%
  distinct(SNP)
# 17 exposure SNPs not present in outcome
...
## Remove APOE region
comb_exposure_dat <- exposure1_stroke_bmi_proxy_clumb_independent 
# %>%
...
mvdat <- mv_harmonise_data(comb_exposure_dat, outcome_dat)
mvdat
...
## Extending these analyses into a multivariable framework we are able to 
## determine the independent effects of SA and OBESITY on CHD. 
## We observed that higher genetically predicted SA remains significantly 
## causally associated with increased CHD risk, while the causal effect of  
## OBESITY on CHD is now non-significant. ????
