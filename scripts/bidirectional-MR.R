...
stroke_exp_dat <- read_tsv(stroke_path)
head(stroke_exp_dat)

## Format data to TwoSampleMR format
exposure <- stroke_exp_dat %>%
  format_data(.,
              type = "exposure",
              snps = NULL,
              header = TRUE,
...
# Now that have identified outliers, we can re-run our core MR methods with outliers removed. We join the radial MR results 
# to our harmonized dataset and modify the mr_keep variable to flag outliers for removal.
