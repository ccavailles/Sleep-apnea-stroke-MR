...
## MR using potentially overlapping samples ----
...
exposure_test$n <- 351316 

outcome$samplesize.outcome <- 1308460

outcome_test <- outcome %>% 
  rename("chr" = "chr.outcome",
         "pos" = "pos.outcome",
         "alt" = "other_allele.outcome",
         "ref" = "effect_allele.outcome",
         "beta" = "beta.outcome",
         "se" = "se.outcome",
         "n" = "samplesize.outcome")

mrlap <- MRlap(exposure = exposure_test,
               exposure_name = "exposure",
               outcome = outcome_test,
               outcome_name = "outcome",
               ld = "data/GenomicSEMFiles/eur_w_ld_chr",
               hm3 = "data/GenomicSEMFiles/w_hm3.snplist")
mrlap
...
res_single_outlier <- mr_singlesnp(mr_dat_outlier, all_method = c("mr_ivw_fe", "mr_egger_regression", 
                                                                  "mr_weighted_median", "mr_weighted_mode")) %>% as_tibble()

funnel_p <- mr_funnel_plot(res_single_outlier)
funnel_out_p <- funnel_p[[1]] + theme_bw() + 
  guides(color=guide_legend(ncol =1)) + 
  theme(
    text = element_text(size = 8), 
